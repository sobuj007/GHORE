...
use App\Models\OrdersVendorNew;
...
class OrdersNewController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($validated['service_product_ids'] as $index => $serviceProductId) {
            $serviceProduct = ServiceProduct::find($serviceProductId);
            $productQuantity = $validated['product_quantities'][$index];
            $serviceQuantity = $validated['service_quantities'][$index];
            $totalProductAmount += $productQuantity * $serviceProduct->product_price;
            $totalServiceAmount += $serviceQuantity * $serviceProduct->service_price;
        }
        ...
        foreach ($request->agent_id as $index => $vendorId) {
            ...
            $orderVendor = OrdersVendorNew::create([
                'order_id' => $order->id,
                'vendor_id' => $vendorId,
        ...
        foreach ($request->service_product_ids as $key => $serviceProductId) {
            $service = ServiceProduct::find($serviceProductId);

            OrdersItems::create([
                'order_id' => $order->id,
                'vendor_id' => $service->agent_id,
                ...
                'payable' => ($request->service_quantities[$key] * $service->service_price) + ($request->product_quantities[$key] * $service->product_price),
            ]);
        }
